...
final getMyDonationsUsecaseProvider = Provider<GetMyDonationsUsecase>((ref) {
  final donationRepository = ref.read(donationRepositoryProvider);
  return GetMyDonationsUsecase(donationRepository: donationRepository);
});

class GetMyDonationsUsecase implements UsecaseWithoutParams<List<DonationEntity>> {
  final IDonationRepository _donationRepository;

  GetMyDonationsUsecase({required IDonationRepository donationRepository})
    : _donationRepository = donationRepository;

  @override
  Future<Either<Failure, List<DonationEntity>>> call() {
    return _donationRepository.getMyDonations();
  }
}
